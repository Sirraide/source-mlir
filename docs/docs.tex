...
\noindent An \emph{initialiser call} may occur only as the argument to an (implicit)
\emph{return expression} \sref[expr.return] of a struct initialiser. The initialiser
must be followed

Its effects are
to delegate the initialisation of the current object to the

\subsection{Summary [expr.temp.mat]}
\textit{Temporary materialisation} is the act of converting an rvalue to an lvalue
by allocating a temporary object and evaluating the rvalue with the temporary as
its result object \sref[expr.rvalue].

If the rvalue expression is the result of performing lvalue-to-rvalue conversion
on a materialised temporary of the same type, the temporary is reused directly,
and no additional materialisation is performed. Additionally, if the object is
already an lvalue, no materialisation is performed.

Temporary materialisation is performed on rvalue expressions in the following contexts:
\begin{enumerate}
\item In a \emph{member access} \sref[expr.access] on the \emph{object} of the
      access, if it is of structure type.
\end{enumerate}
