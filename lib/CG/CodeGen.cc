...
struct CodeGen {
    ...
    StringMap<hlir::FuncOp> declared_procedures;
    ...
    switch (ctor->ctor_kind) {
        ...
        case ConstructKind::ArrayInitialiserCall: {
            ...
            Create<hlir::ConstructOp>(
                loc,
                addr,
                mod->save(ctor->init()->mangled_name).value(),
                arguments,
                ctor->elems()
            );
        } break;
...
auto src::CodeGen::Destructor(Type type) -> std::optional<StringRef> {
    ...
    if (auto s = dyn_cast<StructType>(type))
        return s->deleter ? std::optional{mod->save(s->deleter->mangled_name).value()} : std::nullopt;
...
auto src::CodeGen::EmitReference([[maybe_unused]] mlir::Location loc, src::Expr* decl) -> mlir::Value {
    ...
    if (auto p = dyn_cast<ProcDecl>(decl)) return Create<mlir::func::ConstantOp>(
        loc,
        Ty(p->type),
        mlir::SymbolRefAttr::get(mctx, mod->save(p->mangled_name).value())
    );
...
void src::CodeGen::GenerateProcedure(ProcDecl* proc) {
    ...
    /// at most once.
    auto mangled_name = mod->save(proc->mangled_name).value();
    if (declared_procedures.contains(mangled_name)) {
        auto func_op = declared_procedures[mangled_name];
    ...
    proc->mlir_func = func;
    declared_procedures[mangled_name] = func;
