...
class InvokeInitialiserExpr :public TypedExpr {
public:
    /// The arguments to the initialiser.
    SmallVector<Expr*> args;

    /// The initialiser being invoked.
    ProcDecl* initialiser{};

    InvokeInitialiserExpr(SmallVector<Expr*> args, Location loc)
        : TypedExpr(Kind::InvokeInitialiserExpr, Type::Unknown, loc),
          args(std::move(args)) {}

    /// RTTI.
    static bool classof(const Expr* e) { return e->kind == Kind::InvokeInitialiserExpr; }
};
...
/// Raw :: ( ... ) literal.
class RawLitExpr : public TypedExpr {
public:
    ...
    RawLitExpr(SmallVector<Expr*> args, Location loc)
        : TypedExpr(Kind::RawLitExpr, Type::Unknown, loc),
          args(std::move(args)) {}
};
...
public:
    ...
    /// Get the parent struct, if this is a member function.
    readonly_decl(StructType*, smp_parent);

    /// Get what type of member function this is.
    readonly_decl(SpecialMemberKind, smp_kind);
...
public:
    ...
    /// RTTI.
    static bool classof(const Expr* e) { return e->kind == Kind::EnumType; }
};
...
public:
    ...
    SpecialMemberKind smp_kind = SpecialMemberKind::None;
