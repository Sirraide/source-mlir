...
public:
    explicit Result() requires std::is_void_v<ValueType> : data(std::monostate{}) {}
    ...
    readonly(bool, is_diag, return std::holds_alternative<Diag>(data));

    /// Check if the result holds a value.
    readonly(bool, is_value, return std::holds_alternative<ValueType>(data));
    ...
    auto operator>>=(Callable&& cb) -> make_result_t<std::invoke_result_t<Callable, ValueType&>> {
        ...
        else return ResultType{std::invoke(std::forward<Callable>(cb), value())};
    }
